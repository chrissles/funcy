...
funcit <- function(data,  k, 
	...
	if(missing(methods))
		...
		if("fitfclust" %in% usedMethods){
			...
			RES[[indx]] <-
				parallelFct(fitfclustWrapper(data=data, 
																		 k=k, 
																		 reg=reg, 
																		 regTime=regTime, 
																		 fpcCtrl=fpcCtrl,
		...
		if("distclust" %in% usedMethods){
			...
			RES[[indx]] <- 
				parallelFct(distclustWrapper(data=data, 
																		 k=k,
																		 reg=reg, 
																		 regTime=regTime, 
																		 fpcCtrl=fpcCtrl,
		...
		if("iterSubspace" %in% usedMethods){
			...
			RES[[indx]] <- 
				parallelFct(iterSubspaceWrapper(data=data, 
																				k=k, 
																				reg=reg, 
																				regTime=regTime,
																				fpcCtrl=fpcCtrl,
		...
		if("funclust" %in% usedMethods){
			...
			RES[[indx]] <-
				parallelFct(funclustWrapper(data=data, 
																		k=k, 
																		reg=reg, 
																		regTime=regTime,
																		funcyCtrlMbc=funcyCtrl,
		...
		if("funHDDC" %in% usedMethods){
			...
			RES[[indx]] <-
				parallelFct(funHDDCWrapper(data=data, 
																	 k=k,
																	 reg=reg, 
																	 regTime=regTime,
																	 funcyCtrlMbc=funcyCtrl, ...))
		...
		if("fscm" %in% usedMethods){
			...
			RES[[indx]] <-
				parallelFct(fscmWrapper(data=data, 
																k=k, 
																reg=reg,
																regTime=regTime,
																funcyCtrlMbc=funcyCtrl, ...))
		...
		if("waveclust" %in% usedMethods){
			...
			RES[[indx]] <-
				parallelFct(waveclustWrapper(data=data, 
																		 k=k, 
																		 reg=reg,
																		 regTime=regTime,
																		 funcyCtrlMbc=funcyCtrl, ...))
